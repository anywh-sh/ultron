...
- **Migração em andamento**: a maior parte das telas ainda tem português cravado, de antes do i18n existir. Cada fase do redesign move a copy dos componentes que ela reescreve, e a varredura final pega o resto — então código novo já nasce com dicionário, e código tocado migra junto. Achar um literal em português numa tela não é permissão pra adicionar outro.

...
**Frente ativa: port iOS.** Phase 0 de investigação concluída ([`docs/22`](./docs/22-mobile-ios-investigacao.md), 3 spikes validados: toolchain, UI nativa Liquid Glass via plugin Swift, reconexão em background). Plano de implementação aprovado e em execução ([`docs/23`](./docs/23-mobile-ios-plano-implementacao.md)) — Fase A (fundação do projeto iOS) concluída, próxima é a Fase B (gating de voz/titlebar/tabs pro iOS). **Próxima sessão: veja `docs/23` pra continuar da Fase B em diante.** Não precisa reler `docs/00` a `docs/22` pra continuar.

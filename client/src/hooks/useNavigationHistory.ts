...
/**
 * Pilha de navegação estilo browser: cada mudança de perfil/aba ativa (por
 * ação do usuário) empilha uma entrada, cortando qualquer "forward" que
 * existisse. `goBack`/`goForward` movem um ponteiro pela pilha sem empilhar
 * nada — `notifyLocationChanged` ignora a próxima chamada logo depois de um
 * dos dois (via `skipNextRef`), porque senão a própria restauração de
 * profile+aba viraria uma entrada nova (docs/21).
 */
export function useNavigationHistory(): {
  ...
  const notifyLocationChanged = useCallback((location: NavLocation) => {
    ...
    if (current && current.profileId === location.profileId && current.tabId === location.tabId) return;

 ...
 * per-group piece) so a future multi-group layout can mount one of these per
 * column without duplicating the drag/rename/context-menu wiring. Must
 * render under a Radix `Tabs` ancestor (`TabsTrigger` needs its context) —
 * that root stays one level up since it also has to wrap the panel layer.
 */
export function TabGroupStrip({ tabs, onClose, onReorder, onRenameSession, onDelete }: TabGroupStripProps) {
